...
`@apipay.vn/mcp` is a Model Context Protocol (MCP) server for ApiPay. It exposes tools over stdio so AI assistants (Claude, Cursor, Codex, Command Code, Grok Build, Gemini, Copilot, OpenCode, and others) can create sandbox payments, manage Commerce products and prices, list bank accounts and transactions, manage webhooks, and query metrics.
...
The server registers 23 tools. MCP clients automatically prefix tool names with the server key `apipay` (for example `apipay_list_banks` in OpenCode, `mcp__apipay__list_banks` in Claude Code).
...
3. **Create a product and price** (optional) — Call `create_product` with `defaultPriceData`, or `list_prices` with a `lookupKey` you already stored.
4. **Create a payment request** — Call `create_payment` with `bankPublicId` and either `amount` (for example `"50000"`) or `priceId`. A `publicId` starting with `test_pr_...` is returned.
5. **Simulate customer payment** — Call `simulate_payment` with `publicId: "test_pr_..."` to test settlement and webhook delivery.
6. **Check history** — Call `list_webhook_deliveries` and `list_transactions` to verify the transaction was recorded.
