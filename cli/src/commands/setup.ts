...
import {getAuth, getApiKey, isLoggedIn, markStepComplete} from "../lib/config.js";
...
export default class Setup extends BaseCommand {
	static override description =
		"Set up ApiPay interactively — login, generate API key, add bank, register webhook";
	...
	async run(): Promise<void> {
		...
		if (!tokenValid) {
			...
			console.log(
				chalk.gray(
					"  Sign in to your account or create a new one at https://my.apipay.vn/signup \n",
				),
			);
			await Login.run([]);
			console.log("");
		...
		if (!isLoggedIn()) {
			this.error(
				"Login is required before proceeding. Run `apipay login` first.",
				{exit: 1},
			);
		}
		console.log(stepLabel(2, totalSteps, "Generate API Key"));
		console.log(
			chalk.gray("  Create an API key to authenticate your integration.\n"),
		);
		...
		console.log(
			chalk.gray("  Connect a bank account to start receiving payments.\n"),
		);
		...
		try {
			...
			existingBanks = Array.isArray(data)
				? data
		...
		console.log(
			chalk.gray(
				"  Set up a webhook to receive transaction notifications.\n",
			),
		);
		...
		try {
			...
			existingWebhooks = Array.isArray(data)
				? data
				: (data?.data ?? data ?? []);
		...
		if (existingWebhooks.length > 0) {
			...
			const addAnother = await promptConfirm(
				"Would you like to add another webhook?",
				false,
			);
